...
def _find_runit_sv_dir() -> Path | None:
    for candidate in [Path("/run/runit"), Path("/run/sv"), Path("/service")]:
        if candidate.is_dir():
            return candidate
    return None


def _read_pid_from_supervise(name: str, sv_dir: Path) -> int | None:
    pid_path = sv_dir / name / "supervise" / "pid"
    try:
...
def _read_runit_description(name: str) -> str | None:
    desc_path = Path("/etc/sv") / name / "description"
    try:
...
def _get_runit_status_via_sv(name: str) -> str | None:
    output = _run_cmd(["sv", "status", name])
    if output is None:
        return None
    # sv status output: "run: <name>: (pid <pid>) <seconds>"
    # or: "down: <name>: <seconds>"
...
def get_runit_services() -> list[ServiceInfo]:
    ...
    if not sv_dir.is_dir():
        return services
    try:
        run_sv_dir = _find_runit_sv_dir()
        running: set[str] = set()
        if run_sv_dir is not None:
            for entry in run_sv_dir.iterdir():
                if entry.is_dir():
        ...
        for entry in sorted(sv_dir.iterdir()):
            ...
            pid = _read_pid_from_supervise(svc_name, run_sv_dir) if run_sv_dir else None
            if status == "stopped":
                sv_status = _get_runit_status_via_sv(svc_name)
                if sv_status:
                    status = sv_status
            description = _read_runit_description(svc_name)
            services.append(
                ServiceInfo(
                    name=svc_name,
                    status=status,
